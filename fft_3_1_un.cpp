...
void new_fft(cp *a,int limit,int type){
    ...
    for(int i=0;i<half;i++){
        ...
        cp Wn(cos(2*pi*i/limit),type*sin(2*pi*i/limit));
        //if(type==-1)
         //   x=conj(x);
        c[i] = a[i] + Wn * a[i + half]; //根据之前推出的结论计算
        c[i + half] = a[i] - Wn * a[i + half];
    }
    for(int i=0;i<limit;i++)
        a[i]=c[i];
}
...
int main(){
    ...
    */
    ...
    for(int i=0;i<n;i++)
        cout<<a[i]<<endl;
    new_fft(a,limit,-1);
    for (int i = 0; i < limit-1; i++) printf("%d ", (int)(a[i].real() / limit + 0.5));
    return 0;
}
